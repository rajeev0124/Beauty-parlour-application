  ...
  async generateInvoicePDF(data: InvoiceData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Invoice label
        doc.fontSize(20).text('INVOICE', doc.page.width - 150, 50, { width: 100, align: 'right' });
        ...
        doc.font('Helvetica').text(new Date(data.date).toLocaleDateString('en-IN'), rightX + 100, detailsY + 15);
        ...
        tableHeaders.forEach((header, i) => {
          doc.text(header, xPos, tableTop + 3, { width: colWidths[i], align: i > 1 ? 'right' : 'left' });
          xPos += colWidths[i];
        });
        ...
        data.items.forEach((item, index) => {
          ...
          xPos += colWidths[0];
          doc.text(item.description || '-', xPos, yPos, { width: colWidths[1] });
          xPos += colWidths[1];
          doc.text(item.quantity.toString(), xPos, yPos, { width: colWidths[2], align: 'right' });
          xPos += colWidths[2];
          doc.text(`₹${item.unitPrice.toFixed(2)}`, xPos, yPos, { width: colWidths[3], align: 'right' });
          xPos += colWidths[3];
        ...
        doc.moveTo(totalsX - 20, yPos - 10).lineTo(doc.page.width - 50, yPos - 10).stroke('#E5E7EB');
        
        doc.font('Helvetica').fontSize(10);
        doc.text('Subtotal:', totalsX, yPos);
        doc.text(`₹${data.subtotal.toFixed(2)}`, totalsX + 80, yPos, { width: 70, align: 'right' });
        
        if (data.discount && data.discount > 0) {
          yPos += 20;
          doc.fillColor('#22C55E').text('Discount:', totalsX, yPos);
          doc.text(`-₹${data.discount.toFixed(2)}`, totalsX + 80, yPos, { width: 70, align: 'right' });
          doc.fillColor('#333333');
        }
        ...
        if (data.tax && data.tax > 0) {
          ...
          doc.text(`₹${data.tax.toFixed(2)}`, totalsX + 80, yPos, { width: 70, align: 'right' });
        }
        
        yPos += 25;
        doc.moveTo(totalsX - 20, yPos - 5).lineTo(doc.page.width - 50, yPos - 5).stroke('#333333');
        
        doc.font('Helvetica-Bold').fontSize(12);
        doc.text('Total:', totalsX, yPos);
        doc.text(`₹${data.total.toFixed(2)}`, totalsX + 80, yPos, { width: 70, align: 'right' });
        ...
        if (data.notes) {
          ...
          doc.font('Helvetica').fontSize(9).text(data.notes, 50, yPos + 15, { width: 300 });
        }
        ...
        doc.text('Thank you for choosing Beauty Parlour!', 50, footerY + 15, { align: 'center', width: doc.page.width - 100 });
        doc.text('123 Beauty Lane, Fashion District, City - 560001 | +91 98765 43210 | hello@beautyparlour.com', 
